...
import { DynamicValue, ValoreTabellato } from '../model/tabellato-models';
import { StoreService } from '../services/storeService';
import { ArrayUtils } from './array-utils.service';
import { AppCostants } from './dgue-constants';
...
export class FormBuilderUtilsService extends SdkBaseService {
    ...
    public populateListCode(valoriTabellati: IDictionary<Array<ValoreTabellato>>, field: SdkFormBuilderField, restValue: any, mapping: boolean, annoInizio?: number): Array<any> {
        ...
        if (comboCode === 'sino') {
            set(field, 'data', restValue === false ? this.translateService.instant('COMBOBOX.NO') : this.translateService.instant('COMBOBOX.SI'));
            mapping = false;
        } else {
        ...
        if (!isEmpty(field.itemsProviderCode)) {
            ...
            if (field.type === 'MULTISELECT'){
                ...
                if(jsonLots.length == 1){
                    set(field,'data', [{ key: jsonLots[0].numLot,
    ...
    private elaborateSection(field: any, populate: boolean, customParamsFunction?: CustomParamsFunction, restObject?: any, providerArgs?: IDictionary<any>, infoBox?: boolean): SdkFormBuilderField {
        ...
        each(field.fieldSections, (one: SdkFormBuilderField) => {
            ...
            if (one.type === 'FORM-SECTION') {
                one = this.elaborateSection(one, populate, customParamsFunction, restObject, providerArgs, infoBox);
                let importedXml = this.store.viewElement(AppCostants.IMPORTED_XML);
                let whoImportedXml = this.store.viewElement(AppCostants.WHO_IMPORTED);
                let userInfo=this.store.viewElement(AppCostants.STORE_USR);
                if (!isEmpty(importedXml) && !isEmpty(whoImportedXml)) {                   
                    if (!isEmpty(importedXml[one.code]) && importedXml[one.code].exists === false && userInfo !== 'sa') {
                        one.visible = false;
                        //console.log('visible false ' + one.code);
                    } else {
                        //console.log('visible true ' + one.code);
                    }                    
                }
            } else if (one.type === 'FORM-GROUP') {
                one = this.elaborateGroup(one, populate, customParamsFunction, restObject, providerArgs, infoBox);
    ...
    public getField(formConfig: SdkFormBuilderConfiguration, fieldCode: string): SdkFormBuilderField {
        ...
        if (isObject(field.visibleCondition.and)) {
            each(field.visibleCondition.and, (condition: SdkFormBuilderVisibleCondition, key: string) => {
                ...
                if (isObject(found)) {
                    ...
                    if (!isEmpty(found.listCode) && isObject(restObject) && !isObject(data) && isEmpty(data)) {
                    ...
                    let item: SdkFormBuilderVisibleCondition = find(field.visibleCondition.and, (cond: SdkFormBuilderVisibleCondition, key: string) => {
                        return cond.visible === true && key !== found.code;
                    });

                    othersVisible = size(field.visibleCondition.and) === 1 || (size(field.visibleCondition.and) > 1 && isObject(item));
    ...
    private verifyVisibleValueCondition(condition: SdkFormBuilderVisibleCondition, actualValue: string | number, type: TSdkFormBuilderFieldType, operationType: string): boolean {
        ...
        each(valoriPossibili, (one: SdkFormBuilderVisibleValue) => {
            ...
            let verificata: boolean = (new Function('return ' + condizioneDaVerificare))();
